# Week 16: 最後衝刺 - 面試技巧與投遞 FAANG

**學習時間**：2026/02/17 - 2026/02/21（35 小時）

## 本週目標
1. 完成所有面試準備
2. 投遞 Google, Amazon 等目標公司
3. 準備 Behavioral Questions
4. 進行高強度模擬面試

## 本週學習內容

### 📝 Behavioral Interview 準備

**時間分配**：8 小時

#### Cracking The Coding Interview

- [ ] **Cracking The Coding Interview Set 1**:
    - [ ] [Gayle L McDowell - Cracking The Coding Interview (video)](https://www.youtube.com/watch?v=rEJzOhC5ZtQ)
    - [ ] [Cracking the Coding Interview with Author Gayle Laakmann McDowell (video)](https://www.youtube.com/watch?v=aClxtDcdpsQ)

- [ ] **Cracking the Facebook Coding Interview**:
    - [ ] [The Approach](https://www.youtube.com/watch?v=wCl9kvQGHPI)
    - [ ] [Problem Walkthrough](https://www.youtube.com/watch?v=4UWDyJq8jZg)

#### Be thinking of for when the interview comes

Think of about 20 interview questions you'll get, along with the lines of the items below. Have at least one answer for each. Have a story, not just data, about something you accomplished.

- [ ] Why do you want this job?
- [ ] What's a tough problem you've solved?
- [ ] Biggest challenges faced?
- [ ] Best/worst designs seen?
- [ ] Ideas for improving an existing product
- [ ] How do you work best, as an individual and as part of a team?
- [ ] Which of your skills or experiences would be assets in the role and why?
- [ ] What did you most enjoy at [job x / project y]?
- [ ] What was the biggest challenge you faced at [job x / project y]?
- [ ] What was the hardest bug you faced at [job x / project y]?
- [ ] What did you learn at [job x / project y]?
- [ ] What would you have done better at [job x / project y]?

#### Have questions for the interviewer

Some of mine (I already may know the answers, but want their opinion or team perspective):

- [ ] How large is your team?
- [ ] What does your dev cycle look like? Do you do waterfall/sprints/agile?
- [ ] Are rushes to deadlines common? Or is there flexibility?
- [ ] How are decisions made in your team?
- [ ] How many meetings do you have per week?
- [ ] Do you feel your work environment helps you concentrate?
- [ ] What are you working on?
- [ ] What do you like about it?
- [ ] What is the work life like?
- [ ] How is the work/life balance?

---

### 🎯 Mock Interviews 密集訓練

**時間分配**：15 小時

本週進行至少 **10 次** 模擬面試（每天 2 次）

#### Mock Interview 平台

- [ ] [Gainlo.co: Mock interviewers from big companies](http://www.gainlo.co/)
- [ ] [Pramp: Mock interviews from/with peers](https://www.pramp.com/)
- [ ] [Interviewing.io: Practice with senior engineers](https://interviewing.io)
- [ ] [Meetapro: Mock interviews with FAANG interviewers](https://meetapro.com/?utm_source=ciu)
- [ ] [Hello Interview: Mock Interviews with Expert Coaches](https://www.hellointerview.com/?utm_source=ciu)

#### 模擬面試安排

**Day 1-2: Coding Interview (8 次)**
- 每次 45 分鐘
- 1-2 道 Medium 或 1 道 Hard
- 專注於溝通與思考過程

**Day 3: Behavioral Interview (2 次)**
- 每次 30 分鐘
- 練習 STAR 方法回答
- 準備你的故事

**Day 4-5: System Design (選修)**
- 如果有申請 Senior 職位
- 練習設計常見系統（URL Shortener, Twitter, etc.）

---

### 💻 LeetCode 最後練習

**時間分配**：8 小時

#### 高頻題目清單（必做）

以下是 Google, Amazon 等公司的高頻題目：

**Array & String**
1. [ ] [238. Product of Array Except Self](https://leetcode.com/problems/product-of-array-except-self/)
2. [ ] [560. Subarray Sum Equals K](https://leetcode.com/problems/subarray-sum-equals-k/)

**Tree & Graph**
1. [ ] [200. Number of Islands](https://leetcode.com/problems/number-of-islands/)
2. [ ] [297. Serialize and Deserialize Binary Tree](https://leetcode.com/problems/serialize-and-deserialize-binary-tree/)

**Dynamic Programming**
1. [ ] [139. Word Break](https://leetcode.com/problems/word-break/)
2. [ ] [72. Edit Distance](https://leetcode.com/problems/edit-distance/)

**Design**
1. [ ] [146. LRU Cache](https://leetcode.com/problems/lru-cache/)
2. [ ] [208. Implement Trie](https://leetcode.com/problems/implement-trie-prefix-tree/)

**Hard**
1. [ ] [23. Merge k Sorted Lists](https://leetcode.com/problems/merge-k-sorted-lists/)
2. [ ] [42. Trapping Rain Water](https://leetcode.com/problems/trapping-rain-water/)

---

### 📧 投遞履歷

**時間分配**：4 小時

#### 本週行動

**Day 1-2: 準備申請材料**
- [ ] 最終檢查履歷
- [ ] 準備 Cover Letter（針對不同公司）
- [ ] 更新 LinkedIn
- [ ] 準備推薦人名單

**Day 3: 投遞 FAANG**
- [ ] Google (優先)
- [ ] Amazon (優先)
- [ ] Meta/Facebook
- [ ] Microsoft
- [ ] Apple

**Day 4: 投遞其他目標公司**
- [ ] Stripe
- [ ] Airbnb
- [ ] Uber
- [ ] Netflix
- [ ] 其他心儀公司

**Day 5: 追蹤與後續**
- [ ] 記錄所有申請
- [ ] 設定提醒追蹤進度
- [ ] 準備 Phone Screen

---

## Behavioral Questions - STAR 方法

### STAR 方法模板

**Situation（情境）**：設定背景
**Task（任務）**：說明你的角色與目標
**Action（行動）**：描述你採取的具體行動
**Result（結果）**：量化成果與學習

### 準備 10 個故事

根據你的 8 年經驗，準備以下類型的故事：

1. **Leadership / Initiative**
   - [ ] 帶領團隊完成專案
   - [ ] 主動提出改進方案

2. **Problem Solving**
   - [ ] 解決複雜的技術問題
   - [ ] 在有限資源下完成任務

3. **Conflict Resolution**
   - [ ] 與同事意見不合的處理
   - [ ] 處理客戶/使用者反饋

4. **Learning & Growth**
   - [ ] 學習新技術的經驗
   - [ ] 從失敗中學習

5. **Impact & Achievement**
   - [ ] 最有成就感的專案
   - [ ] 帶來顯著業務影響的工作

---

## 面試當天準備

### Technical Interview Checklist

**前一天**
- [ ] 複習核心資料結構與演算法
- [ ] 做 2-3 道 LeetCode 熱身
- [ ] 準備好白板/紙筆
- [ ] 早點休息，充足睡眠

**面試前 1 小時**
- [ ] 確認設備（視訊、網路、IDE）
- [ ] 準備水
- [ ] 深呼吸，放鬆心情
- [ ] 複習常用模板

**面試中**
- [ ] 仔細聽題，確認理解
- [ ] 大聲思考，表達過程
- [ ] 先討論解法，再寫程式
- [ ] 測試 edge cases
- [ ] 分析時間空間複雜度
- [ ] 保持積極態度

**面試後**
- [ ] 立即記錄題目與心得
- [ ] 總結可以改進的地方
- [ ] 發送 Thank You Email

---

## 最後檢核清單

### 技術準備 ✅
- [ ] 完成 150+ LeetCode 題目
- [ ] 熟練所有核心資料結構
- [ ] 熟練所有核心演算法
- [ ] 能在 30-45 分鐘內解決 Medium 題目
- [ ] 能清楚分析時間空間複雜度
- [ ] 完成至少 10 次模擬面試

### 非技術準備 ✅
- [ ] 履歷完善且無錯誤
- [ ] 準備好 20 個 Behavioral Questions 答案
- [ ] 準備好 10 個要問面試官的問題
- [ ] LinkedIn 更新完成
- [ ] 推薦人已聯繫

### 心理準備 ✅
- [ ] 對自己的能力有信心
- [ ] 接受可能的失敗
- [ ] 保持積極正面的心態
- [ ] 準備好應對壓力
- [ ] 把面試當成學習機會

---

## 最後的話

### 16 週的學習旅程即將完成

恭喜你堅持到最後！16 週的密集學習非常不容易，但你做到了。

### 記住這些

**你的優勢**：
✅ 8 年實務經驗
✅ iGaming 產業的專業知識
✅ 團隊領導經驗
✅ 完整的演算法訓練

**面試心態**：
1. **不要害怕失敗** - 每次面試都是學習機會
2. **展現真實的自己** - 面試是雙向選擇
3. **相信努力會有回報** - 你已經做了該做的準備
4. **保持謙虛與好學** - 即使進入 FAANG 也要持續學習

### 面試時間軸預估

- **Week 16**: 投遞履歷
- **Week 17-18**: Phone Screen (1-2 輪)
- **Week 19-20**: Technical Phone Interview (1-2 輪)
- **Week 21-22**: Onsite Interview (4-5 輪)
- **Week 23**: Offer & 談判

記得：整個流程可能需要 4-8 週，保持耐心！

---

## Keep Learning!

**Once You've Got The Job**

Congratulations!

Keep learning.

You're never really done.

---

**祝你面試順利！Good luck! 🚀**

---

## 學習心得記錄

### 16 週總結
[記錄這 16 週的完整學習心得]

### 最大的收穫
[記錄最大的收穫是什麼]

### 最困難的部分
[記錄什麼最困難，如何克服]

### 給未來自己的話
[寫給未來自己的鼓勵]

---

**這只是開始，不是結束。永遠保持學習的心態！**
