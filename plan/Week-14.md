# Week 14: Networking 與其他主題

**學習時間**：2026/02/02 - 2026/02/06（30 小時）

## 本週目標
1. 理解網路基礎知識
2. 完成剩餘的學習主題
3. 開始投遞履歷給第二梯次公司

## 本週學習內容

### 📚 Networking

**時間分配**：12 小時

**If you have networking experience or want to be a reliability engineer or operations engineer, expect questions**

- [ ] [Khan Academy](https://www.khanacademy.org/computing/code-org/computers-and-the-internet)
- [ ] [UDP and TCP: Comparison of Transport Protocols (video)](https://www.youtube.com/watch?v=Vdc8TCESIg8)
- [ ] [TCP/IP and the OSI Model Explained! (video)](https://www.youtube.com/watch?v=e5DEVa9eSN0)
- [ ] [Packet Transmission across the Internet. Networking & TCP/IP tutorial. (video)](https://www.youtube.com/watch?v=nomyRJehhnM)
- [ ] [HTTP (video)](https://www.youtube.com/watch?v=WGJrLqtX7As)
- [ ] [SSL and HTTPS (video)](https://www.youtube.com/watch?v=S2iBR2ZlZf0)
- [ ] [SSL/TLS (video)](https://www.youtube.com/watch?v=Rp3iZUvXWlM)
- [ ] [HTTP 2.0 (video)](https://www.youtube.com/watch?v=E9FxNzv1Tr8)
- [ ] [Video Series (21 videos) (video)](https://www.youtube.com/playlist?list=PLEbnTDJUr_IegfoqO4iPnPYQui46QqT0j)
- [ ] [Subnetting Demystified - Part 5 CIDR Notation (video)](https://www.youtube.com/watch?v=t5xYI0jzOf4)
- [ ] **Sockets**:
    - [ ] [Java - Sockets - Introduction (video)](https://www.youtube.com/watch?v=6G_W54zuadg&t=6s)
    - [ ] [Socket Programming (video)](https://www.youtube.com/watch?v=G75vN2mnJeQ)

---

### 📚 Final Review 開始

**時間分配**：8 小時

This section will have shorter videos that you can watch pretty quickly to review most of the important concepts. It's nice if you want a refresher often.

- [ ] Series of 2-3 minutes short subject videos (23 videos)
    - [Videos](https://www.youtube.com/watch?v=r4r1DZcx1cM&list=PLmVb1OknmNJuC5POdcDv5oCS7_OUkDgpj&index=22)
- [ ] Series of 2-5 minutes short subject videos - Michael Sambol (48 videos):
    - [Videos](https://www.youtube.com/@MichaelSambol)
    - [Code Examples](https://github.com/msambol/dsa)

---

### 💻 LeetCode 綜合練習 + 模擬面試

**時間分配**：10 小時

本週開始進行模擬面試準備：
- 每天進行 1 次 45 分鐘的模擬面試
- 隨機選擇 1-2 道 Medium 題目
- 限時完成，練習白板寫程式
- 練習大聲說出思考過程

#### 推薦練習題目（混合主題）

**必做經典題（10 題）**：
1. [ ] [1. Two Sum](https://leetcode.com/problems/two-sum/)
2. [ ] [3. Longest Substring Without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/)
3. [ ] [15. 3Sum](https://leetcode.com/problems/3sum/)
4. [ ] [20. Valid Parentheses](https://leetcode.com/problems/valid-parentheses/)
5. [ ] [53. Maximum Subarray](https://leetcode.com/problems/maximum-subarray/)
6. [ ] [121. Best Time to Buy and Sell Stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/)
7. [ ] [206. Reverse Linked List](https://leetcode.com/problems/reverse-linked-list/)
8. [ ] [238. Product of Array Except Self](https://leetcode.com/problems/product-of-array-except-self/)
9. [ ] [242. Valid Anagram](https://leetcode.com/problems/valid-anagram/)
10. [ ] [70. Climbing Stairs](https://leetcode.com/problems/climbing-stairs/)

---

### 📝 履歷與面試準備

**時間分配**：5 小時

#### Update Your Resume

- [ ] See Resume prep information in the books: "Cracking The Coding Interview" and "Programming Interviews Exposed"
- [ ] ["This Is What A GOOD Resume Should Look Like" by Gayle McDowell](https://www.careercup.com/resume)
- [ ] ["Step-by-step resume guide" by Tech Interview Handbook](https://www.techinterviewhandbook.org/resume/guide)

#### Interview Process & General Interview Prep

- [ ] [How to Pass the Engineering Interview in 2021](https://davidbyttow.medium.com/how-to-pass-the-engineering-interview-in-2021-45f1b389a1)
- [ ] [Demystifying Tech Recruiting](https://www.youtube.com/watch?v=N233T0epWTs)
- [ ] **How to Get a Job at the Big 4**:
    - [ ] [How to Get a Job at the Big 4 - Amazon, Facebook, Google & Microsoft (video)](https://www.youtube.com/watch?v=YJZCUhxNCv8)
    - [ ] [How to Get a Job at the Big 4.1 (Follow-up video)](https://www.youtube.com/watch?v=6790FVXWBw8&feature=youtu.be)

---

## 本週行動計劃

### Day 1-2: Networking 學習
專注於網路基礎知識，特別是 TCP/IP, HTTP, HTTPS

### Day 3-4: Final Review + 模擬面試
開始使用 Final Review 影片複習，每天 1 次模擬面試

### Day 5: 履歷更新 + 開始投遞
- 更新履歷
- 準備 Cover Letter
- 開始投遞第二梯次公司（非 FAANG）

---

## 學習建議

**註：** 本週開始前請先完成上週未完成的內容與複習

### Day 1 (Monday) - 6 小時
- 上午（3h）：Week-13 複習 + Networking 影片（Khan Academy, UDP/TCP）
- 下午（3h）：Networking 影片（TCP/IP, OSI Model, Packet Transmission）

### Day 2 (Tuesday) - 6 小時
- 上午（3h）：Networking 影片（HTTP, SSL/HTTPS, HTTP 2.0）
- 下午（3h）：Networking 影片（Video Series, Subnetting, Sockets）

### Day 3 (Wednesday) - 6 小時
- 上午（3h）：Final Review 影片（2-3 minutes short videos）
- 下午（3h）：Final Review 影片（Michael Sambol videos）

### Day 4 (Thursday) - 6 小時
- 上午（3h）：Resume 更新 + LeetCode 綜合練習 × 2
- 下午（3h）：面試技巧學習 + LeetCode × 2

### Day 5 (Friday) - 6 小時
- 上午（3h）：LeetCode 綜合練習 × 3
- 下午（3h）：模擬面試 × 1 + 本週總複習

---

## 本週檢核清單

### 知識理解
- [ ] 理解 OSI 7 層模型
- [ ] 理解 TCP vs UDP
- [ ] 理解 HTTP/HTTPS 的運作
- [ ] 理解 Socket Programming 基礎

### 面試準備
- [ ] 履歷更新完成
- [ ] 完成至少 3 次模擬面試
- [ ] 準備好 Behavioral Questions 答案
- [ ] 開始投遞履歷

### 解題能力
- [ ] 完成至少 10 道經典題目
- [ ] 能在 45 分鐘內解決 1-2 道 Medium 題目
- [ ] 能清楚表達思考過程

---

## 重要提醒

### 開始投遞履歷
本週開始可以投遞第二梯次公司（非 FAANG），理由：
1. 練習面試流程
2. 降低焦慮感
3. 如果拿到 offer 可以作為談判籌碼
4. 真實面試經驗是最好的練習

### 推薦投遞公司類型
- 新創獨角獸（Stripe, Airbnb, Uber 等）
- 科技公司（Microsoft, Apple 等）
- 有好工程文化的中型公司

---

## 學習心得記錄

### 本週收穫
[在這裡記錄你的學習心得]

### 面試準備心得
[記錄面試準備的過程與心得]

---

**下週預告**：Week 15 將進行 Final Review 與密集 LeetCode 練習
